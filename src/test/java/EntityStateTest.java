...
public class EntityStateTest {
    ...
    void test1() {
        ...
        try {
            ...
            Memo memo = new Memo(); // 객체를 하나 만든 상태 // 비영속 상태 (JPA가 관리하고 있는 Entity가 아님, 순수한 Entity클래스 상태)
            memo.setId(1L);
            memo.setUsername("Robbie");
            memo.setContents("비영속과 영속 상태");

            em.persist(memo);  // 영속성 컨택스트에 저장됨. 비영속상태 entity를 entity Manager를 통해 영속성 컨텍스트에 저장. 영속 상태
